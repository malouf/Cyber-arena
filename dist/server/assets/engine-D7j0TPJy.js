import { b as selectEnemyActions, o as obstacles } from "./ArenaView-BIFbbCQh.js";
...
function resolveTurn(pState, eState, pQueue, turnState, params = {}) {
  const events = [];
  const {
  ...
  [pStats, eStats].forEach((stats) => {
    stats.effects.filter((e) => e.type === "dot").forEach((dot) => {
  ...
  const eQueue = eStats.hp > 0 ? selectEnemyActions(eStats, pStats, interactables) : [];
  const checkInteractables = (entity, pos) => {
    const triggered = [];
    for (const interactable of interactables) {
      if (interactable.pos.x === pos.x && interactable.pos.y === pos.y && (interactable.triggeredBy === entity || interactable.triggeredBy === "both")) {
        triggered.push(interactable);
      }
    }
    return triggered;
  };
  const applyDamage = (target, amount, isMelee) => {
    ...
    if (shieldIndex !== -1 && finalDamage > 0) {
      ...
      if (shield.value !== void 0) shield.value -= absorbed;
  ...
  for (let i = 0; i < maxSteps; i++) {
    const pAction = pQueue[i] ? { ...pQueue[i], entity: "player" } : null;
    const eAction = eQueue[i] ? { ...eQueue[i], entity: "enemy" } : null;
    const stepActions = [];
    ...
    for (const action of stepActions) {
      ...
      if (action.type === "move") {
        ...
        const ability = action.ability || action;
            ...
            for (const n of neighbors) {
              ...
              if (d < minDist) {
                ...
                const isInteractableWall = interactables.some(
                  (int) => int.type === "wall" && int.pos.x === n.x && int.pos.y === n.y
                );
  ...
  [pStats, eStats].forEach((stats) => {
    stats.effects = stats.effects.map((e) => ({ ...e, duration: e.duration - 1 })).filter((e) => e.duration > 0);
  });
