    ...
    <div className="bg-neutral-950 border border-neutral-900 p-6 flex-shrink-0">
      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-600 mb-4 flex justify-between">
        ...
        {activeCommand && !disabled && (
          <span className="text-red-500 animate-pulse">Waiting for target...</span>
        )}
      </h3>
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
        ...
        <div className="pt-2 border-t border-neutral-900 space-y-2">
          {equippedAbilities.map((ability, index) => {
            ...
            const isActive =
              activeCommand?.type === 'ability' && activeCommand.ability.id === ability.id
            ...
            return (
              <div key={`${ability.id}-${index}`} className="relative group">
                <button
                  ...
                  `}
                >
                  {isUlt && <div className="absolute top-0 left-0 bottom-0 w-1 bg-red-600" />}

                  <div className="flex justify-between items-center w-full mb-1">
                    <span className="font-bold uppercase text-[10px] tracking-widest flex items-center gap-2">
                      {ability.name}
                      {isBase && <span className="text-[8px] text-neutral-500">(Base)</span>}
                      {isUlt && <span className="text-[8px] text-red-500">(Ult)</span>}
                    </span>
                    <div className="flex gap-2 font-mono text-[9px]">
                      {currentCooldown > 0 && <span className="text-red-500">CD: {currentCooldown}</span>}
                      {ability.paCost > 0 && <span>{ability.paCost}PA</span>}
                      {ability.manaCost > 0 && (
                        <span className="text-blue-500">{ability.manaCost}M</span>
                      )}
                    </div>
                  </div>
                </button>
