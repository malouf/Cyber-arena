      ...
      <div className="bg-neutral-950 border border-neutral-900 p-4 flex flex-col h-1/2">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-2">
          {actionQueue.length === 0 ? (
            <p className="text-[9px] text-neutral-700 font-mono">No commands queued.</p>
          ) : (
            actionQueue.map((action, i) => (
              ...
              >
                <span className="text-neutral-300">
                  {(i + 1).toString().padStart(2, '0')} {action.name.toUpperCase()}
                </span>
                <div className="flex gap-2">
                  <span className="text-neutral-600">Init:{action.initiative}</span>
                  {action.pmCost > 0 && <span className="text-neutral-500">-{action.pmCost}PM</span>}
                  {action.paCost > 0 && <span className="text-neutral-300">-{action.paCost}PA</span>}
                  {action.manaCost > 0 && <span className="text-blue-500">-{action.manaCost}M</span>}
                </div>
              </div>
            ))
          )}
