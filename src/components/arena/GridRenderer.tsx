      ...
      <div className="flex flex-col gap-1 items-center m-auto relative min-w-[800px] lg:min-w-[1000px] pb-12">
        {rows.map((y) => (
          <div 
            ...
            style={{ transform: `translateX(${y % 2 !== 0 ? '2.2%' : '-2.2%'})` }}
          >
            {cols.map((x) => {
              const cell = { x, y }
              ...
              return (
                ...
                >
                  {/* Visual Effects */}
                  {visualEffects.filter(e => e.x === x && e.y === y).map(effect => (
                    <div key={effect.id} className={`absolute z-50 animate-bounce font-black text-lg ${effect.color}`} style={{ textShadow: '0 0 4px black' }}>
                      {effect.text}
                    </div>
                  ))}
                  ...
                  {inValidRange && !isHoveredValid && isVisible && (
                    <div className="absolute inset-0 bg-white/5 z-10 pointer-events-none" style={{ clipPath: hexStyle.clipPath }} />
                  )}
                  {isHoveredValid && isVisible && (
                    <div className="absolute inset-0 bg-red-600/20 z-10 pointer-events-none" style={{ clipPath: hexStyle.clipPath }} />
                  )}

                  {/* Player Marker */}
                  {isUnit && (
                    <div className={`absolute z-30 w-[40%] h-[40%] rounded-full bg-white shadow-[0_0_15px_rgba(255,255,255,0.8)] ${isResolving ? 'animate-pulse' : ''}`} />
                  )}
