...
type GameStore = {
  ...
  // Initialization
  initializeGame: (player: EntityState, enemy: EntityState, initialLogs?: Array<string>) => void
  ...
  // Cell interaction
  handleCellClick: (cell: { x: number; y: number }, buildPassives: Array<string>) => void
  ...
  applyEvent: (event: Exclude<CombatEvent, { type: 'delay' }>, effectId?: number) => void
  ...
  immer((set, get) => ({
    ...
    initializeGame: (player, enemy, initialLogs) =>
      set((state) => {
        ...
        state.ui.logs = initialLogs ?? ['System online. Select a command to begin sequence.']
      }),
    ...
    handleCellClick: (cell, buildPassives) =>
      set((state) => {
        ...
        const isOccupiedByEnemy = cell.x === server.enemy.pos.x && cell.y === server.enemy.pos.y
        ...
        if (ui.activeCommand.type === 'move') {
          if (dist > 0 && dist <= simStats.pm && !isOccupiedByEnemy && !isWall) {
            ...
            state.ui.logs = ['Invalid move target.', ...state.ui.logs].slice(0, 15)
          }
          return
        }
        ...
        if (currentCooldown > 0) {
          state.ui.logs = [`${ability.name} is on cooldown for ${currentCooldown} more turns.`, ...state.ui.logs].slice(0, 15)
          return
        }

        const currentUses = server.usesThisTurn[ability.id] || 0
        const queuedUses = server.actionQueue.filter(
          (action: Action) => action.type === 'ability' && action.ability?.id === ability.id,
        ).length

        if (ability.maxUsesPerTurn && currentUses + queuedUses >= ability.maxUsesPerTurn) {
          state.ui.logs = [`Max uses per turn reached for ${ability.name}.`, ...state.ui.logs].slice(0, 15)
          return
        }

        if (!(dist <= effectiveRange && (!isWall || ability.type === 'buff'))) {
          state.ui.logs = [`Target out of range for ${ability.name}.`, ...state.ui.logs].slice(0, 15)
          return
        }

        if (!(simStats.pa >= ability.paCost && simStats.mana >= ability.manaCost)) {
          state.ui.logs = [`Insufficient resources for ${ability.name}.`, ...state.ui.logs].slice(0, 15)
          return
        }

        if (ability.type === 'move_attack' && (isOccupiedByEnemy || isWall)) {
          state.ui.logs = ['Cannot leap onto occupied cell.', ...state.ui.logs].slice(0, 15)
          return
        }
    ...
    removeEffect: (effectId) =>
      set((state) => {
        state.ui.visualEffects = state.ui.visualEffects.filter((e) => e.id !== effectId)
      }),
    ...
    runTurnResolution: async (player, enemy, queue, turnState) => {
      ...
      const { events, nextTurnState } = resolveTurn(player, enemy, queue, turnState)
      let effectId = 1
