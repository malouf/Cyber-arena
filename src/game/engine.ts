...
export type CombatEvent = 
  ...
  | { type: 'stats', entity: 'player' | 'enemy', hp?: number, pa?: number, pm?: number, mana?: number }
...
export function resolveTurn(
  ...
  pQueue: Action[],
  turnState: TurnState
): { events: CombatEvent[], nextTurnState: TurnState } {
  ...
  let newCooldowns = { ...turnState.cooldowns }
  let newUses = { ...turnState.usesThisTurn }
  ...
  if (eStats.hp > 0) {
    ...
    if (dist > 1) {
      ...
      const isAIWall = obstacles.some(o => o.x === newEnemyPos.x && o.y === newEnemyPos.y)
      if ((newEnemyPos.x !== pPos.x || newEnemyPos.y !== pPos.y) && !isAIWall) {
        eQueue.push({ type: 'move', target: newEnemyPos, initiative: 100, name: 'Move', paCost:0, pmCost:1, manaCost:0 })
      }
      if (getDistance(newEnemyPos, pPos) <= 1) {
        eQueue.push({ type: 'ability', target: pPos, name: 'Basic Attack', initiative: 50, paCost:0, pmCost:0, manaCost:0, ability: { id: 'bot_attack', name: 'Basic Attack', damage: 15, range: 1, type: 'attack' } as Ability })
      }
    } else {
      eQueue.push({ type: 'ability', target: pPos, name: 'Basic Attack', initiative: 50, paCost:0, pmCost:0, manaCost:0, ability: { id: 'bot_attack', name: 'Basic Attack', damage: 15, range: 1, type: 'attack' } as Ability })
    }
  ...
  const applyDamage = (target: 'player' | 'enemy', amount: number, isMelee: boolean) => {
    if (target === 'player') {
      let finalDamage = amount
      if (pStats.passives.includes('heavy_plating')) finalDamage = Math.max(0, finalDamage - 3)
      pStats.hp -= finalDamage
      events.push({ type: 'effect', pos: { ...pPos }, text: `-${finalDamage}`, color: 'text-red-500' })
      ...
      if (pStats.passives.includes('thorns') && isMelee) {
        eStats.hp -= 5
        events.push({ type: 'effect', pos: { ...ePos }, text: `-5`, color: 'text-red-500' })
        events.push({ type: 'stats', entity: 'enemy', hp: eStats.hp })
        events.push({ type: 'log', text: '> Thorns reflected 5 DMG!' })
      }
    } else {
      eStats.hp -= amount
      events.push({ type: 'effect', pos: { ...ePos }, text: `-${amount}`, color: 'text-red-500' })
      events.push({ type: 'stats', entity: 'enemy', hp: eStats.hp })
    }
  }
  ...
  for (let i = 0; i < maxSteps; i++) {
    const pAction = pQueue[i] ? { ...pQueue[i], entity: 'player' as const } : null
    const eAction = eQueue[i] ? { ...eQueue[i], entity: 'enemy' as const } : null
    ...
    for (const action of stepActions) {
      ...
      if (action.type === 'move') {
        if (isPlayer) {
          ...
          events.push({ type: 'stats', entity: 'player', pm: pStats.pm, pa: pStats.pa, mana: pStats.mana })
        } else {
          ePos = action.target
          events.push({ type: 'move', entity: 'enemy', pos: { ...ePos } })
        }
        events.push({ type: 'log', text: `> ${isPlayer ? 'Player' : 'Enemy'} moved to [${action.target.x}, ${action.target.y}] (Init ${action.initiative})` })
      } 
        ...
        if (isPlayer && ability.id) {
          ...
          if (ability.maxUsesPerTurn) newUses[ability.id] = (newUses[ability.id] || 0) + 1
        }
        
        let dmg = ability.damage || 0
        if (isPlayer && pStats.passives.includes('momentum') && ability.type.includes('attack')) {
        ...
        if (ability.id === 'impulse' && isPlayer) {
          if (currentDist <= effectiveRange && action.target.x === targetPos.x && action.target.y === targetPos.y) {
            ...
            events.push({ type: 'log', text: `> Impulsion pushes Enemy to [${ePos.x}, ${ePos.y}]! ${dmg} DMG.` })
            if (pStats.passives.includes('drain_force')) {
              ...
              events.push({ type: 'log', text: `> Drain de Force restored 1 Mana!` })
            }
          } else {
        ...
        if (ability.type === 'move_attack') {
          if (isPlayer) {
            ...
            pPos = action.target
            events.push({ type: 'move', entity: 'player', pos: { ...pPos } })
            events.push({ type: 'log', text: `> ${ability.name} leaps to [${pPos.x}, ${pPos.y}]!` })
            
            if (getDistance(pPos, ePos) <= 1) {
              applyDamage('enemy', dmg, true)
              events.push({ type: 'log', text: `> ${ability.name} hits Enemy for ${dmg} DMG.` })
            } else {
              events.push({ type: 'log', text: `> No adjacent targets for ${ability.name}.` })
            }
          }
        } else {
          if (currentDist <= effectiveRange) {
            if (action.target.x === targetPos.x && action.target.y === targetPos.y) {
              if (isPlayer) {
                applyDamage('enemy', dmg, currentDist <= 1)
                events.push({ type: 'log', text: `> ${ability.name} hits Enemy! ${dmg} DMG.` })
              } else {
                applyDamage('player', dmg, currentDist <= 1)
                events.push({ type: 'log', text: `> Enemy uses ${ability.name}! ${dmg} DMG.` })
              }
            } else {
              events.push({ type: 'log', text: `> ${isPlayer?'Player':'Enemy'} attacks empty cell [${action.target.x}, ${action.target.y}].` })
            }
          } else {
            events.push({ type: 'log', text: `> ${ability.name} FAILED: Target now out of range.` })
          }
        }
      }
    }
  ...
  if (cellsMovedThisTurn === 0 && pStats.passives.includes('flow_state')) {
    ...
    events.push({ type: 'log', text: `> Flow State active: Next turn spells gain +1 Range.` })
  }
  ...
  pStats.mana = Math.min(pStats.maxMana, pStats.mana + 1)
  events.push({ type: 'stats', entity: 'player', pa: pStats.pa, pm: pStats.pm, mana: pStats.mana })
