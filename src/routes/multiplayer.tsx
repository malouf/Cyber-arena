...
function MatchRoom({
  ...
  const handleCellClick = (x: number, y: number) => {
    ...
    setQueue((previous) => [
      ...previous,
      { type: selectedCommand, target: { x, y } },
    ]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError(
        error instanceof Error ? error.message : "Turn submission failed.",
      );
    } finally {
        ...
        <header className="border-b border-neutral-900 pb-4 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
          <div>
            ...
            <p className="text-xs uppercase tracking-[0.2em] text-neutral-500 mt-1">
              Turn {match.turnNumber} • {match.phase.replaceAll("_", " ")} • You
              are {match.yourSlot}
            </p>
          </div>
          <Link
            ...
            <div className="grid gap-1 min-w-[520px]">
              {boardRows.map((y) => (
                <div key={y} className="grid grid-cols-10 gap-1">
                  {boardCols.map((x) => {
                    const isPlayer =
                      match.playerState.pos.x === x &&
                      match.playerState.pos.y === y;
                    const isEnemy =
                      match.enemyState.pos.x === x &&
                      match.enemyState.pos.y === y;
                    return (
                      <button
                        key={`${x}-${y}`}
          ...
          <aside className="space-y-4">
            <section className="border border-neutral-900 bg-neutral-950 p-4 space-y-3">
              ...
              <div className="text-xs font-mono space-y-2">
                <p>
                  You: HP {match.playerState.hp}/{match.playerState.maxHp} • PA{" "}
                  {match.playerState.pa} • PM {match.playerState.pm}
                </p>
                <p>
                  Enemy: HP {match.enemyState.hp}/{match.enemyState.maxHp} • PA{" "}
                  {match.enemyState.pa} • PM {match.enemyState.pm}
                </p>
              </div>
              <div className="text-[10px] uppercase tracking-[0.2em] text-neutral-500">
                P1 submitted: {match.player1Submitted ? "yes" : "no"} • P2
                submitted: {match.player2Submitted ? "yes" : "no"}
              </div>
            </section>
              ...
              <div className="space-y-2 max-h-32 overflow-auto pr-1">
                {queue.length === 0 ? (
                  ...
                  queue.map((action, index) => (
                    <div
                      key={index}
                      className="text-[11px] font-mono border border-neutral-800 p-2"
                    >
            ...
            <section className="border border-neutral-900 bg-neutral-950 p-4">
              <h2 className="text-[10px] uppercase tracking-[0.2em] text-neutral-500 mb-2">
                ...
                {match.latestEvents.length === 0 ? (
                  ...
                  match.latestEvents.map((event, index) => {
                    ...
                    if (event.type === "attack") {
                      return (
                        <p key={index}>
                          • {event.entity} attacked [{event.target.x},
                          {event.target.y}] →{" "}
